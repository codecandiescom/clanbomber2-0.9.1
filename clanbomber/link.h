/*
	$Id: link.h,v 1.2 2003/06/12 00:39:46 xmassx Exp $
...
//		CL_Link *cur = chunk_alloc.get();
	...
	CL_Link *add(CL_Link *after, TYPE *item)
	{
	  CL_Link *cur = new CL_Link;
      cur->instance = item;
	...
	//mass: needed for sorted insertion
    //: if 'after' is NULL this verion of add() simply prepends
    //  the new item to the list, else the new link is appended
    //  after the link 'after'.
    //!param: after - The link to add the new item after.
    //!param: item - Item to be added.
    //!retval: Link-item which has been created and added.

    CL_Link *get_link(int number)
    {
      if (number<0 || number>num_items) return NULL;
      
      CL_Link *cur_instance = first;
      for (int i=0; i<number; i++) cur_instance=cur_instance->next;
    ...
    //!param: number - Position of link to return.
    //!retval: Link if found, NULL otherwise.
