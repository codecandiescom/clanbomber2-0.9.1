...
Server::Server(ClanBomberApplication* app)
{
    ...
    bool no_error=true;
    if(!init_my_name()) {
        cout<<"(!) clanbomber: error while initializing server name"<<endl;
        no_error=false;
    }
    if(no_error && !init_my_ip()) {
     	cout<<"(!) clanbomber: error while initializing server ip"<<endl;
        no_error=false;
    }
    if(no_error && !init_tcp_socket()) {
        cout<<"(!) clanbomber: error while initializing server tcp socket"<<endl;
        no_error=false;
    }
    if(no_error && !init_udp_socket()) {
        cout<<"(!) clanbomber: error while initializing server udp socket"<<endl;
        no_error=false;
    }
    if(no_error) {
...
Server::~Server()
{
    ...
    while(true) {
        ...
        delete client_addresses[i];
        delete client_ips[i];
        delete client_names[i];
        shutdown(client_tcp_sockets[i], SHUT_RDWR);
        close(client_tcp_sockets[i]);
        client_tcp_sockets[i]=-1;
    }
}
...
bool Server::add_client(int socket)
{
    int free_index=get_next_free_client_index();
    keep_client_alive(free_index);
    ...
    sockaddr_in* act_addr_in=new sockaddr_in;
    memset((char*)act_addr_in, 0, sockaddr_size);
    int pres=getpeername(socket, (sockaddr*)act_addr_in, &sockaddr_size);
    if(pres<0) {
        printf("(!) clanbomber: server error while adding client <%s>.\n", client_names[free_index]);
        return false;
    }
    client_ips[free_index]=strdup(inet_ntoa(act_addr_in->sin_addr));
    act_addr_in->sin_family=PF_INET;
    act_addr_in->sin_port=htons(NET_CLIENT_UDP_PORT);
    client_addresses[free_index]=(sockaddr*)act_addr_in;
    struct hostent *he=NULL;
...
int Server::get_client_index_by_ip(in_addr ip)
{
    ...
    while(true) {
        ...
        if(strcmp(client_ips[i], ip_str)==0) {
            return i;
        }
    }
    return -1;
...
void Server::send_SERVER_FULL_BOMBER_CONFIG()
{
    ...
    for(int i=0;i<8;i++) {
        ...
        tmp[pos++]=Config::bomber[i].get_name().get_length();
        int namelen=Config::bomber[i].get_name().get_length();
        int intnamelen=0;
        int* namebuf=Client::pack_string(Config::bomber[i].get_name().get_string(), namelen, &intnamelen);
...
void Server::send_SERVER_ADD_BOMBER()
{
    ...
    while(bomber_object_counter.next()!=NULL) {
        if(bomber_object_counter()!=NULL) {
            tmp[pos++]=bomber_object_counter()->get_object_id();
            tmp[pos++]=((bomber_object_counter()->get_x() << 16) | bomber_object_counter()->get_y());
            tmp[pos++]=((bomber_object_counter()->get_color() << 16) | bomber_object_counter()->get_team());
            tmp[pos++]=((bomber_object_counter()->get_number() << 16) | bomber_object_counter()->get_points());
            bomber_to_add++;
        }
    }
    tmp[2]=bomber_to_add;
...
void Server::send_SERVER_UPDATE_BOMBER()
{
    ...
    while(bomber_object_counter.next()!=NULL) {
        if(bomber_object_counter()!=NULL) {
            tmp[pos++]=((bomber_object_counter()->get_x() << 16) | bomber_object_counter()->get_y());
            tmp[pos++]=(((bomber_object_counter()->get_server_send_dir()+100) << 16) | bomber_object_counter()->get_object_id());
            bomber_to_update++;
        }
    }
    ClanBomberApplication::unlock();
...
void Server::send_SERVER_UPDATE_MAP()
{
    ...
    pos+=intmaplen;
    delete mapbuf;
    int namelen=cb_app->map->get_name().get_length();
    tmp[pos++]=namelen;
    int intnamelen=0;
    int* namebuf=Client::pack_string(cb_app->map->get_name().get_string(), namelen, &intnamelen);
    memcpy(&tmp[pos], namebuf, intnamelen*4);   
    delete namebuf;
    pos+=intnamelen;
    int c=0;
    ...
    while(bomber_object_counter.next()!=NULL) {
        if(bomber_object_counter()!=NULL) {
            ...
            tmp[pos++]=((bomber_object_counter()->get_points() << 16) | bomber_object_counter()->get_object_id());
        }
    }
...
void Server::analyze_game_mode()
{
    ...
    if(in_demo_mode) {
        cout<<"(+) clanbomber: server is in demo mode.. (no human players)"<<endl;
    }
}
...
void Server::delete_client(int client_index)
{
    ...
    delete client_addresses[client_index];
    client_addresses[client_index]=NULL;
    ...
    for(int i=0;i<8;i++) {
        if(Config::bomber[i].get_client_index()==client_index) {
            ...
            while(my_bomber_object_counter.next()!=NULL) {
                if(my_bomber_object_counter()->get_number()==i) {
                    my_bomber_object_counter()->set_disconnected();
                    my_bomber_object_counter()->die();
                    send_SERVER_DISCONNECT_CLIENT(my_bomber_object_counter()->get_object_id());
                                                  my_bomber_object_counter.remove();
                }
            }
        }
    }
...
bool Server::is_free_client_index(int client_index)
{
    return (client_ips[client_index]==NULL &&
            client_names[client_index]==NULL &&
            client_addresses[client_index]==NULL &&
            client_tcp_sockets[client_index]<0);
}
...
bool Server::map_already_exists(char* map_name)
{
...
void Server::recv_CLIENT_BOMBER_DIR(int* data, int* pos)
{
    ...
    while(bomber_object_counter.next() != NULL) {
        if(bomber_object_counter()!=NULL) {
            if(bomber_object_counter()->get_number()==cl_nr) {
                bomber_object_counter()->set_server_dir(cl_dir);
                bomber_object_counter()->set_local_dir(cl_dir); 
                if(cl_dir!=DIR_NONE) {
                    bomber_object_counter()->set_cur_dir(cl_dir);
                }
                break;
            }
        }
...
{
    MapEntry* me=new MapEntry(CL_String("dummy"), false);
    ...
    me->set_name(CL_String(map_name));
    me->set_author(CL_String(map_author));
    ...
    if(map_already_exists(map_name)) {
        ...
        if(!map_already_exists(me->get_name().get_string())) {
            me->write_back();
        }
    }
    else {
...
void Server::recv_CLIENT_HANDSHAKE(int* data, int* pos, int from_client_index)
{
    ...
    if(client_protocol_version!=NET_PROTOCOL_VERSION) {
        cout<<"(!) clanbomber: connection from <"<<client_names[from_client_index]<<"> refused, protocol version "<<client_protocol_version<<" (required "<<NET_PROTOCOL_VERSION<<")."<<endl;
        delete_client(from_client_index);
        return;
    }
