...
Client::Client(ClanBomberApplication* app, char* srv_name)
{
    ...
    current_chat_message=CL_String("");
    ...
    if(!init_my_name()) {
        cout<<"(!) clanbomber: error while initializing client name"<<endl;
        no_error=false;
    }
    if(no_error && !init_my_ip()) {
        cout<<"(!) clanbomber: error while initializing client ip"<<endl;
        no_error=false;
    }
    if(no_error && !init_tcp_socket()) {
        cout<<"(!) clanbomber: error while initializing client tcp socket"<<endl;
        no_error=false;
    }
    if(no_error && !init_udp_socket()) {
        cout<<"(!) clanbomber: error while initializing client udp socket"<<endl;
        no_error=false;
    }
    if(no_error && !connect_to_server()) {
         cout<<"(!) clanbomber: client error while connecting to server <"<<srv_name<<">"<<endl;
         no_error=false;
    }
    if(no_error) {
        cout<<"(+) clanbomber: client connected to server <"<<server_ip<<">"<<endl;
        delete last_error;
        last_error=NULL;
    }
...
Client::~Client()
{
    ...
    delete server_name;
    delete server_ip;
    shutdown(my_tcp_socket, SHUT_RDWR);
    close(my_tcp_socket);
    my_tcp_socket=-1;
    shutdown(my_udp_socket, SHUT_RDWR);
    close(my_udp_socket);
    my_udp_socket=-1;
    Chat::reset();
}
...
bool Client::connect_to_server()
{
    ...
    if(he==NULL) {
        printf("(?) clanbomber: could not find host entity for '%s'. Interpreting as IP...\n", server_name);
        is_error=true;
    }
    if(!is_error) {
        ...
        int cres=connect(my_tcp_socket, (struct sockaddr*)&server_addr, sizeof(server_addr));
        if(cres<0) {
            printf("(?) clanbomber: tcp connection to '%s' failed, interpreting as IP...\n", server_name);
            is_error=true;
        }
    }
    if(is_error) {
        in_addr_t iat=inet_addr(server_name);
        if(iat==INADDR_NONE) {
            return false;
        }
        memset((char*)&server_addr, 0, sizeof(server_addr));
        server_addr.sin_family=PF_INET;
        server_addr.sin_addr.s_addr=iat;
        server_addr.sin_port=htons(NET_SERVER_TCP_PORT);
        int cres=connect(my_tcp_socket, (struct sockaddr*)&server_addr, sizeof(server_addr));
        if(cres<0) {
            sprintf(last_error, "!!! tcp connection to '%s' failed, no game there?", server_name);
        }
        else {
            is_error=false;
        }
...
bool Client::init_udp_socket()
{
    ...
    /*int cres=connect(my_udp_socket, (struct sockaddr*)&server_address, sizeof(server_address));
    if(cres<0) {
        sprintf(last_error, "!!! udp connection to '%s' failed, no game there?", server_name);
    }*/
    return true;
}
...
void Client::send_SERVER_BOMBER_CONFIG(bool enable, int skin, int team, int controller, int rel_pos, char* name)
{
    ...
    for(int j=0;j<bufsize;j++) {
 	...
 	int bytes_sent=sendto(my_udp_socket, buf, bufsize*4, 0, (sockaddr*)&server_address, addr_len);
    total_bytes_sent+=bytes_sent;
}

void Client::send_CLIENT_BOMBER_CONFIG(bool enable, int skin, int controller, int rel_pos, char* name)
{
...
void Client::send_CLIENT_BOMBER_BOMB(int pwr, int obj_id) {
    ...
    int bytes_sent=sendto(my_udp_socket, tmp, bufsize*4, 0, (sockaddr*)&server_address, addr_len);
    total_bytes_sent+=bytes_sent;
}
...
void Client::send_CLIENT_CHAT_MESSAGE(int autom)
{
    ...
    int msglen=current_chat_message.get_length();
    if(msglen<1) {
        return;
    }
    bool sendit=false;
    char* msg=current_chat_message.get_string();
...
void Client::send_CLIENT_MAP_EXCHANGE(unsigned int map_checksum)
{
    ...
    pos+=intmaplen;
    delete mapbuf;
    int namelen=cb_app->map->get_current_map()->get_name().get_length();  
    tmp[pos++]=namelen;
    int intnamelen=0;
    int* namebuf=pack_string(cb_app->map->get_current_map()->get_name().get_string(), namelen, &intnamelen);
    memcpy(&tmp[pos], namebuf, intnamelen*4);
    delete namebuf;
    pos+=intnamelen;
    int authorlen=cb_app->map->get_current_map()->get_author().get_length();
    tmp[pos++]=authorlen;
    int intauthorlen=0;
    int* authorbuf=pack_string(cb_app->map->get_current_map()->get_author().get_string(), authorlen, &intauthorlen);
    ...
    int bytes_sent=sendto(my_udp_socket, tmp, pos*4, 0, (sockaddr*)&server_address, addr_len);
    total_bytes_sent+=bytes_sent;
}
...
int* Client::pack_string(char* name, int bytelen, int* intlen)
{
...
void Client::disconnect_from_server()
{
    if(server_keep_alive_timer.elapsed()>NET_CLIENT_MILLISECONDS_KEEP_SERVER_ALIVE)
    {
        ...
        if(current_client_info_index_is_mine()) {
            ...
            cout<<"(!) clanbomber: server has disconnected, i am the server now..."<<endl;
            ClanBomberApplication::lock();
        ...
        if(Config::bomber[i].get_client_index()==previous_client_index) {
            if(ClanBomberApplication::is_server()) {
                send_SERVER_BOMBER_CONFIG(true,
                                          ...
                                          i,
                                          Config::bomber[i].get_name().get_string());
            }
...
void Client::recv_SERVER_GAME_START(int* data, int* pos)
{
    ...
    cout<<"(+) clanbomber: server started game.."<<endl;
}
...
void Client::recv_SERVER_FULL_BOMBER_CONFIG(int* data, int* pos)
{
    ...
    for(int i=0;i<8;i++) {
        ...
        (*pos)+=intnamelen;
        CL_String cl_name=CL_String(name);
        int ip_len=data[(*pos)++];
        int intiplen=0;
            ...
            while(bomber_object_counter.next()!=NULL) {
                if(bomber_object_counter()!=NULL) {
                    if(bomber_object_counter()->get_object_id()==object_id) {
                        bomber_object_counter()->set_points(points);
                        bomber_object_counter()->controller->deactivate();
                        found=true;
                        break;
                    }
        ...
        if(bomber!=NULL) {
            ...
            if(cb_app->map->get_maptile(bx, by)!=NULL &&
               cb_app->map->get_maptile(bx, by)->bomb!=NULL) {
                ...
                cb_app->objects.add(cb_app->map->get_maptile(bx, by)->bomb);
                int x=bx;
        ...
        if(!ClanBomberApplication::is_server()) {
            ...
            while(game_object_counter.next()!=NULL) {
                if(game_object_counter()!=NULL) {
                    if(game_object_counter()->get_object_id()==an_obj_id) {
                        CLIENTMSG(" (%p) ===> %s", game_object_counter(), GameObject::objecttype2string(game_object_counter()->get_type()));
                        game_object_counter()->delete_me=true;
                        if(game_object_counter()->get_type()==GameObject::BOMB) {
                            game_object_counter()->get_maptile()->bomb=NULL;
                        }
                        found=true;
                        break;
                    }
            ...
            if(!found) {
                ...
                while(bomber_object_counter.next()!=NULL) {
                    if(bomber_object_counter()!=NULL) {
                        if(bomber_object_counter()->get_object_id()==an_obj_id) {
                            CLIENTMSG(" (%p) ===> %s", bomber_object_counter(), GameObject::objecttype2string(bomber_object_counter()->get_type()));
                            bomber_object_counter()->delete_me=true;
                            found=true;
                            break;
                        }
                    }
...
void Client::recv_SERVER_HANDSHAKE(int* data, int* pos)
{
    ...
    if(!ClanBomberApplication::is_server()) {
        if(server_protocol_version!=NET_PROTOCOL_VERSION) {
            cout<<"(!) clanbomber: connection to <"<<server_name<<"> refused, protocol version "<<server_protocol_version<<" required (have "<<NET_PROTOCOL_VERSION<<")."<<endl; 
